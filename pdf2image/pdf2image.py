...
poppler_dir_path = os.getcwd() + "/poppler"
...
def convert_from_path(pdf_path, dpi=200, output_folder=None, first_page=None, last_page=None, fmt='ppm', thread_count=1, userpw=None, use_cropbox=False, strict=False, transparent=False, output_file=str(uuid.uuid4())):
    ...
    for i in range(thread_count):
        ...
        if use_pdfcairo:
            args = [poppler_dir_path + '/pdftocairo'] + args
        else:
            args = [poppler_dir_path + '/pdftoppm'] + args
...
def _page_count(pdf_path, userpw=None):
    try:
        if userpw is not None:
            proc = Popen([poppler_dir_path + "/pdfipdfnfo", pdf_path, '-upw', userpw], stdout=PIPE, stderr=PIPE)
        else:
            proc = Popen([poppler_dir_path + "/pdfinfo", pdf_path], stdout=PIPE, stderr=PIPE)
...
def page_count(pdf_file_local_path):
    count = _page_count(pdf_file_local_path)
    os.remove(pdf_file_local_path)
    return count
